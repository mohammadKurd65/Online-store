...
// import withAdminAuth from "../components/AdminRoute";
...
return function AdminProtectedComponent(props) {
    ...
    return userRole === "admin" ? <Component {...props} /> : null;
};
}

// کامپوننت SalesBarChart
...
return (
    ...
    {products.map((product) => (
        <div key={product._id} className="overflow-hidden rounded border shadow transition hover:shadow-md">
        ...
        <div className="p-4">
            ...
            <button className="px-4 py-2 w-full text-white bg-blue-500 rounded hover:bg-blue-600">
                ویرایش
            </button>
            </a>
            {canDeleteUsers && (
            <button className="px-4 py-2 mt-2 w-full text-white bg-red-500 rounded hover:bg-red-600">
                حذف
            </button>
            )}
        </div>
...
useEffect(() => {
    ...
    fetchData();
}, [ setStats, setRecentOrders, setProducts]);
